...
export default function MeetingSummariesPage({ 
    ...
    // Current Year's Month List
    const activeYearObject = yearsData?.years?.find(y => y.year === selectedYear);
        ...
        <div ref={pageContainerRef} className="flex-1 flex flex-col overflow-y-auto bg-stage-bg pb-12 w-full select-none animate-fade-in scroll-fade-top relative">
            ...
            {selectedMeetingMeta ? (
                        ...
                        {fullMeetingDetail && (
                            <>
                                ...
                                <div className="pb-4 sm:pb-5 mb-5 sm:mb-6">
                                    <div className="inline-block bg-slate-100 text-slate-700 text-[10px] sm:text-xs font-bold px-2.5 py-1 rounded tracking-wider uppercase mb-2 border border-slate-200">
                                        MEETING SUMMARY
                                    </div>
                                    <h1 className="text-2xl sm:text-3xl md:text-4xl font-extrabold text-slate-900 tracking-tight leading-tight mb-4">
                                        {formatLongMeetingDate(fullMeetingDetail.dateFormatted || fullMeetingDetail.date || fullMeetingDetail.title)}
                                    </h1>
                                            ...
                                            <div className="grid grid-cols-2 sm:flex items-center gap-2 flex-1 sm:flex-initial">
                                                {prevMeeting ? (
                                                    <button 
                                                        ...
                                                        title={`Previous Day: ${formatMeetingDate(prevMeeting.dateFormatted || prevMeeting.date || prevMeeting.title)}`}
                                                    >
                                                        ← Previous Day
                                                    </button>
                                                ) : (
                                                ...
                                                {nextMeeting ? (
                                                    <button 
                                                        ...
                                                        title={`Next Day: ${formatMeetingDate(nextMeeting.dateFormatted || nextMeeting.date || nextMeeting.title)}`}
                                                    >
                                                        Next Day →
                                                    </button>
                                                ) : (
                ...
                <div className="pt-20 sm:pt-22 md:pt-24 px-3 sm:px-5 md:px-8 pb-12 max-w-5xl mx-auto w-full flex flex-col gap-6">
                    ...
                    <div className="bg-white border border-border-light rounded-2xl p-5 sm:p-7 shadow-card flex flex-col gap-4">
                            ...
                            <div>
                                <div className="flex items-center gap-2 mb-1">
                                    ...
                                    <span className="text-xs font-semibold text-slate-400">
                                        • {yearsData?.totalMeetings || 0} Total Meetings
                                    </span>
                                </div>
                    ...
                    <div className="flex flex-col gap-3.5">
                        ...
                        <div className="flex items-center justify-between pt-2">
                            ...
                            <span className="text-xs font-semibold text-slate-500">
                                {isSearching 
                                    ? `${searchResults.length} match(es) across archive`
                                    : `${monthData?.meetings?.length || 0} Record(s)`}
                            </span>
                        </div>
                        ...
                        {(!isLoadingMonth || isSearching) && (
                            <div className="flex flex-col gap-2.5">
                                {(isSearching ? searchResults : (monthData?.meetings || [])).map((meeting) => {
                                    const shortDate = formatShortMeetingDate(meeting.dateFormatted || meeting.date || meeting.title);
